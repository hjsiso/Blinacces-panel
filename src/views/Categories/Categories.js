...
class Categories extends Component {
  ...
  delItem(item, e) {
    ...
    if (category == item.categoryName) {
      if (
        confirm("Seguro que desea eliminar la categoria " + category + " ?")
      ) {
        const itemsRef = firebase
          .database()
          .ref("categories/" + item.id)
          .remove();
        this.setState({
          currentItem: "",
          categoryName: "",
      ...
      <div className="animated fadeIn">
        <form className="form-inline mb-2" onSubmit={this.handleSubmit}>
          ...
          <input
            ...
            required
          />
          <div class="invalid-feedback">Please provide a valid Categoria.</div>
          <button
            type="submit"
            className={
              !this.state.showEditOpts
                ? "btn btn-outline-success btn-sm mr-2 float-left visible"
                : "btn btn-outline-success btn-sm mr-2 invisible"
            }
          >
          ...
          <button
            ...
            className={
              this.state.showEditOpts
                ? "btn btn-outline-danger btn-sm mr-2 float-left visible"
                : "btn btn-outline-success btn-sm mr-2 invisible"
            }
            onClick={e => this.hideEditOpts(e)}
          >
                ...
                <tr key={item.id}>
                  ...
                  <td>
                    <button
                      ...
                      onClick={e => this.editItem(item, e)}
                    >
                      <i className="sm icon-pencil" />
                    </button>
                    <button
                      type="button"
                      className="btn btn-outline-danger btn-sm mr-2 float-right"
                      onClick={e => this.delItem(item, e)}
                    >
                      <i className="sm icon-trash" />
                    </button>
                  </td>
                </tr>
              );
